function [fb, b, cost_history] = minimize_energy_legendre(g, varargin)
    % 能量最小化函数（勒让德版本）
    % 交替优化: min ||g - fb - b||^2 + η||Dg - Dfb - Db||^2
    %
    % 输入:
    %   g - 输入图像
    %   可选参数:
    ...
    %       'tol' - 收敛容差 (默认: 1e-6)
    %       'display' - 是否显示信息 (默认: true)
    %       'poly_degree' - 勒让德阶数 (默认: 3)
    %       'num_blocks_m' - 垂直分块数 (默认: 2)
    %       'num_blocks_n' - 水平分块数 (默认: 2)
    %
    % 输出:
    %   fb - 校正后的图像
    %   b - 偏置场
    ...
    % 设置勒让德拟合参数
    legendre_param.num_blocks_m = params.num_blocks_m;
    legendre_param.num_blocks_n = params.num_blocks_n;
    legendre_param.poly_degree = params.poly_degree;
    legendre_param.save_blocks = false;
    
    % 使用勒让德曲面拟合初始化偏置场
    b_initial = legendre_surface_fitting_adaptive(g, legendre_param);
    ...
    for iter = 1:params.max_iter
        ...
        % 步骤1: 固定b，优化fb
        [A_fb, rhs_fb] = build_fb_system(g, b, Dg, Dx, Dy, params.eta, M, N);
        A_fb_reg = A_fb + 1e-8 * speye(size(A_fb));
        fb_vec = A_fb_reg \ rhs_fb(:);
        fb = reshape(fb_vec, M, N);
        
        % 步骤2: 固定fb，优化b（使用勒让德拟合）
        [A_b, rhs_b] = build_b_system(g, fb, Dg, Dx, Dy, params.eta, M, N);
        A_b_reg = A_b + 1e-8 * speye(size(A_b));
        b_vec = A_b_reg \ rhs_b(:);
        b = reshape(b_vec, M, N);
        
        % 使用勒让德拟合平滑
        b = legendre_surface_fitting_adaptive(b, legendre_param);
        
        % 计算成本
        cost = compute_cost(g, fb, b, Dg, Dx, Dy, params.eta);
    ...
    %% 可视化
    if params.display
        visualize_results(g, fb, b, cost_history);
    end
end
...
function [A, rhs] = build_fb_system(g, b, Dg, Dx, Dy, eta, M, N)
    total_pixels = M * N;
...
function [A, rhs] = build_b_system(g, fb, Dg, Dx, Dy, eta, M, N)
    total_pixels = M * N;
    I = speye(total_pixels);
    DTD = Dx' * Dx + Dy' * Dy;
    A = I + eta * DTD;
...
function cost = compute_cost(g, fb, b, Dg, Dx, Dy, eta)
    [M, N] = size(g);
...
function visualize_results(g, fb, b, cost_history)
    figure('Position', [150, 150, 1200, 600]);
    
    subplot(2,3,1); imshow(g, []); title('原始图像'); colorbar;
    subplot(2,3,2); imshow(fb, []); title('校正后图像'); colorbar;
    subplot(2,3,3); imshow(b, []); title('偏置场'); colorbar;
    subplot(2,3,6); surf(fliplr(b)); title('偏置场 3D'); shading interp; axis tight; axis off;
    subplot(2,3,4); imshow(fb+b, []); title('重建图像'); colorbar;
    subplot(2,3,5); semilogy(1:length(cost_history), cost_history, 'LineWidth', 2);
    xlabel('迭代'); ylabel('成本'); title('收敛曲线'); grid on;
end