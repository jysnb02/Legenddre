function I_correct = legendre_surface_fitting_adaptive(img_gray, param)
    ...
    %   img_gray - 输入灰度图像
    %   param - 参数结构体，包含:
    %       num_blocks_m - 垂直方向分块数
    %       num_blocks_n - 水平方向分块数
    %       poly_degree - 多项式阶数 (支持 1,2,3,4,5,...)
    %       save_blocks - 是否保存中间结果
    %       save_path - 保存路径
    % 输出:
    %   I_correct - 拟合后的曲面图像
    ...
    poly_degree = param.poly_degree;  % 用户指定的阶数
    ...
    num_constraints_per_boundary = poly_degree;
    ...
    if num_blocks_n > 1
        for j = 1:num_blocks_m
            ...
            if y_end > y_start
                yb = (y_start : y_end - 1)';
                xb = split_cols(2) * ones(size(yb));
                
                left_basis = generate_legendre_basis_adaptive(xb, yb, ...
                    block_data{j,1}.x_range, block_data{j,1}.y_range, poly_degree);
                right_basis = generate_legendre_basis_adaptive(xb, yb, ...
                    block_data{j,2}.x_range, block_data{j,2}.y_range, poly_degree);
                
                cL_start = ((j-1)*num_blocks_n + 1 - 1) * num_coeffs;
                cR_start = ((j-1)*num_blocks_n + 2 - 1) * num_coeffs;
                
                % 根据阶数动态构造约束
                cons = build_constraints_adaptive(left_basis, right_basis, poly_degree, 'x');
    ...
    if num_blocks_m > 1
        for i = 1:num_blocks_n
            ...
            if x_end > x_start
                xb = (x_start : x_end - 1)';
                yb = split_rows(2) * ones(size(xb));
                
                top_basis = generate_legendre_basis_adaptive(xb, yb, ...
                    block_data{1,i}.x_range, block_data{1,i}.y_range, poly_degree);
                bottom_basis = generate_legendre_basis_adaptive(xb, yb, ...
                    block_data{2,i}.x_range, block_data{2,i}.y_range, poly_degree);
                
                cT_start = (0*num_blocks_n + i - 1) * num_coeffs;
                cB_start = (1*num_blocks_n + i - 1) * num_coeffs;
                
                % 根据阶数动态构造约束
                cons = build_constraints_adaptive(top_basis, bottom_basis, poly_degree, 'y');
    ...
    %% Step 3: 求解带约束的最小二乘问题
    if ~isempty(Aeq)
        KKT = [G, Aeq'; Aeq, zeros(size(Aeq,1))];
        rhs = [h; beq];
        sol = KKT \ rhs;
